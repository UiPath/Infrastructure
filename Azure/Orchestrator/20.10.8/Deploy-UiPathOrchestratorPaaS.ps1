...
function LoginToAzure {
    ...
    if (isAzureUSGovEnvironment) {
        Connect-AzureRmAccount -Identity -AccountId $azureMSIApplicationId -SubscriptionId $azureSubscriptionId -TenantId $azureTenantId -ErrorAction Stop -Environment AzureUSGovernment
        Write-Output "$(Get-Date) Logged into AzureUSGovEnvironment with MSI."
    }
    else {
        ...
        Write-Output "$(Get-Date) Logged into AzureCloud with MSI."
    }
